	...
	calcularTempoOcioso: () => {
		...
		const resultados = saidas.map(saida => {
			...
			if (!dataSaidaBR || isNaN(dataSaidaBR.getTime())) {
				return {
					saidaId: saída.SAIDAID,
					codMotorista: codMotorista,
					numPed: numPed,
					filial: siglaFilial,
					dataSaida: saída.DATASAIDA,
			...
			if (tempoMotoboy && pedidoDisp) {
				...
				// Só consideramos atraso se for MAIOR que 2 minutos
				if (tempoOciosoMinutos <= 2) {
					tempoOciosoMinutos = 0;
				}
		...
		// Apenas registros SEM erro E com atraso maior que 2 minutos
		const resultadosValidos = resultados
		.filter(r => !r.erro && r.tempoOciosoMinutos > 2)
		.sort((a, b) => b.tempoOciosoMinutos - a.tempoOciosoMinutos);
		...
		const saidasComOciosidade = resultadosValidos.length;
		const maxOcioso = resultadosValidos.length > 0 ? Math.max(...resultadosValidos.map(r => r.tempoOciosoMinutos)) : 0;
		const minOcioso = resultadosValidos.length > 0 ? 
					Math.min(...resultadosValidos.map(r => r.tempoOciosoMinutos)) : 0;
